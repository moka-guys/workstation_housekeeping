...
def log_setup(logfile_name, syslog='/dev/log'):
    ...
    logging_config = dict(
        ...
        handlers={
            ...
            'stream_handler': {'class': 'logging.StreamHandler', 'formatter': 'log_formatter', 'level': logging.INFO},
            'file_handler': {'class': 'logging.FileHandler', 'formatter': 'log_formatter', 'level': logging.DEBUG,
                                'filename': logfile_name},
            'syslog_handler': {'class': 'logging.handlers.SysLogHandler', 'formatter': 'log_formatter', 'level': logging.DEBUG,
                                'address': syslog}},
        root={'handlers': ['file_handler', 'stream_handler', 'syslog_handler'], 'level': logging.DEBUG}
        )
