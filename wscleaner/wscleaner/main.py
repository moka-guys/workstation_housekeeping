...
LOGFILE = os.path.join(LOGDIR, f"{TIMESTAMP}_wscleaner.log")  # Path for the application logfile
...
def cli_parser():
    ...
    parser.add_argument('--auth', help='A text file containing the DNANexus authentication token', type=str, default=AUTH_FILE, required=False)
    parser.add_argument('--dry-run', help='Perform a dry run without deleting files', action='store_true', default=False,)
    parser.add_argument('--runfolders_dir', help='A directory containing runfolders to process', required=True)
