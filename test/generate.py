...
def data_test_runfolders():
    ...
    return [
        ("190408_NB551068_0234_AHJ7MTAFXY_NGS265B", os.path.join(PROJECT_DIR, "test/test_dir_1_fastqs.txt")),
        ("190410_NB551068_0235_AHKGMGAFXY_NGS265C", os.path.join(PROJECT_DIR, "test/test_dir_2_fastqs.txt")),
    ]
...
#         test_path = os.path.join(PROJECT_DIR, f"test/data/{runfolder_name}/Data/Intensities/BaseCalls")
#         Path(test_path).mkdir(parents=True, exist_ok=True)
#         # Generate empty fastqfiles in runfolder
#         with open(fastq_list_file) as f:
#             fastq_list = f.read().splitlines()
#             for fastq_file in fastq_list:
#                 Path(test_path, fastq_file).touch(mode=777, exist_ok=True)
